...
def analyze_text(transcript, user_prompt):
    ...
    try:
        if user_prompt == 'summarize':
            prompt = f'write me a detailed blog article about the follow text. Expand the length of the blog relative to the length of the content. Be detail oriented. Begin: {transcript}'
        else:
            prompt = f"Follow this command {user_prompt} in regards to this transcript\n\nTranscript:\n{transcript}"
